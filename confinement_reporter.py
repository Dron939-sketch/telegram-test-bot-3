...
# ============================================
# ГИПНОТЕРАПЕВТИЧЕСКИЙ МОДУЛЬ (сокращенная версия)
# ============================================
...
class Truisms:
    ...
    def about_self(self, feeling: str) -> str:
        return f"Мне кажется, что {feeling}"
...
class PseudoLogic:
    ...
    def cause_effect(self, cause: str, effect: str) -> str:
        return f"{cause}, поэтому {effect}"
    
    def temporal(self, action1: str, action2: str) -> str:
        return f"Когда {action1}, тогда {action2}"
...
class ParadoxCommands:
    ...
    def prohibition(self, forbidden: str) -> str:
        return f"Только не вздумай {forbidden}!"


class EmbeddedSuggestions:
    """Выделение ключевых слов в речи"""
    
    def contextual(self, text: str, suggestion: str) -> str:
        if suggestion in text:
            return text.replace(suggestion, f"**{suggestion}**")
        return text
...
class HypnoQuestions:
    ...
    def rhetorical(self, suggestion: str) -> str:
        return f"Разве {suggestion}?"
    
    def alternative(self, option1: str, option2: str) -> str:
        return f"Ты предпочитаешь {option1} или {option2}?"
    
    def unequal_choice(self, good_option: str, bad_option: str) -> str:
        return f"Тебе {good_option} или лучше {bad_option}?"


class TherapeuticTales:
    """Коллекция терапевтических сказок"""
    
    tales_collection = {
        "fear": {
            "title": "Тигренок",
            "text": "Давным-давно в далекой-далекой стране у самых Синих гор жил маленький полосатый тигренок..."
        },
        "growth": {
            "title": "Сказка о дровосеке",
            "text": "Давным-давно в далекой-далекой стране у самых Синих гор жил дровосек..."
        }
    }
    
    def get_tale_for_issue(self, issue_keyword: str) -> dict:
        issue_lower = issue_keyword.lower()
        if "страх" in issue_lower or "боюсь" in issue_lower:
            return self.tales_collection["fear"]
        return self.tales_collection["growth"]
...
class MiltonModel:
    ...
    def milton_phrase(self, topic: str) -> str:
        return f"И каждый может найти это понимание в своем собственном опыте..."
...
class Anchoring:
    ...
    phrase_anchors = {
        "calm": ["дыши глубже", "всё идёт своим чередом"],
        "confidence": ["ты справишься", "у тебя получится"],
        "action": ["сделай шаг", "начни прямо сейчас"]
    }
    ...
    def get_anchor(self, state: str) -> str:
...
class HypnoOrchestrator:
    ...
    def process_user_input(self, user_id: int, text: str, context: Dict) -> str:
        ...
        response_parts.append(self.anchoring.get_anchor("calm"))
...
async def show_tale(callback: CallbackQuery, state: FSMContext):
    ...
    tale = hypno.tales.get_tale_for_issue("рост")
...
async def handle_question_message(message: Message, state: FSMContext):
    ...
    response = hypno.process_user_input(
        user_id=user_id,
        text=message.text,
...
async def cmd_tale(message: Message):
    ...
    tale = hypno.tales.get_tale_for_issue(text)
