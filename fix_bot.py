#!/usr/bin/env python3
"""
Скрипт для добавления недостающих функций в bot3.py:
- split_long_message
- safe_send_long_message
- обновлённая safe_send_message
"""
...
def main():
    ...
    if "def split_long_message" in content:
        print("⚠️ Функции уже существуют. Проверяем...")
    else:
        print("✅ Добавляем недостающие функции...")
    ...
    # Новые функции для длинных сообщений
    new_functions = '''
...
'''
    ...
    if clean_func_match:
        ...
        # Проверяем, есть ли уже функции
        if "def split_long_message" not in content:
            ...
            print("⚠️ Функции уже существуют, пропускаем добавление")
    else:
    ...
    lines_added = new_functions.count('\n')
    print(f"\n📊 Статистика:")
    print(f"   - Добавлено строк: ~{lines_added}")
    print(f"   - Новая функция: split_long_message")
    print(f"   - Новая функция: safe_send_long_message")
    print(f"   - Обновлена: safe_send_message")
...
def check_imports():
    ...
    # Проверяем импорт List
    if 'from typing import' in content:
        if 'List' not in content.split('from typing import')[1].split('\n')[0]:
            print("\n⚠️ ВНИМАНИЕ: Возможно отсутствует импорт List")
            print("   Проверьте строку: from typing import Optional, Dict, List, Any, Tuple, Union")
    else:
        print("\n⚠️ Не найден импорт из typing")

if __name__ == "__main__":
    print("🔧 Начинаем исправление bot3.py...")
    print("=" * 50)
    success = main()
    if success:
        print("\n" + "=" * 50)
        print("🎉 ГОТОВО! Бот теперь будет разбивать длинные сообщения на части.")
        print("✅ Первая часть будет с кнопками")
        print("✅ Остальные части - просто текст")
        print("❌ Ошибка 'message is too long' больше не появится")
        
        # Проверяем импорты
        check_imports()
        
        print("\n📋 Чтобы запустить бота:")
        print("   python3 bot3.py")
    else:
