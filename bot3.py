...
{f'<i>Рост:</i> {["низкий", "ниже среднего", "средний", "выше среднего", "высокий"][physical["height"]-1]}'}
{f'<i>Сила:</i> {["слабый", "ниже среднего", "средняя", "выше среднего", "очень сильный"][physical["strength"]-1]}'}
{f'<i>Внешность:</i> {["незаметная", "обычная", "симпатичная", "красивая", "модельная"][physical["looks"]-1]}'''
    """
...
• """ + get_recommendation(dominant, scores, secondary) + """
• Учитывайте свои физические данные — они дают преимущества и ограничения
• Окружение можно менять — то, что работало в детстве, может не работать сейчас
"""
...
def get_recommendation(dominant: str, scores: Dict) -> str:
    ...
    if dominant == "СБ":
        if scores["СБ"] > 5:
            return "Ваша сила — в силе. Но помните, что не все проблемы решаются напором."
        else:
            return "Учитесь отстаивать свои границы, но без лишней агрессии."
    
    elif dominant == "ТФ":
        if scores["ТФ"] > 5:
            return "Вы умеете добывать ресурсы. Не забывайте, что отдых тоже важен."
        else:
            return "Развивайте навыки планирования и накопления."
    
    elif dominant == "УБ":
        if scores["УБ"] > 5:
            return "Ваш ум — главный инструмент. Не забывайте про практику."
        else:
            return "Учитесь анализировать ситуации, ищите закономерности."
    
    else:  # ЧВ
        if scores["ЧВ"] > 5:
            return "Вы прекрасно строите связи. Не забывайте про искренность."
        else:
            return "Развивайте коммуникативные навыки, учитесь знакомиться."
