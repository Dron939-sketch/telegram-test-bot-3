...
ПОЛНАЯ ВЕРСИЯ с 8 вопросами и проверкой консистентности
"""
...
# ══════════════════════════════════════════════
#  ВОПРОСЫ ТЕСТА (8 вопросов на вектор)
# ══════════════════════════════════════════════
...
# ─── НОВЫЕ ФУНКЦИИ ДЛЯ ПРОВЕРКИ КОНСИСТЕНТНОСТИ ───────────────────────────
def check_consistency(scores_list: list) -> bool:
    """
    Проверяет не хаотичны ли ответы.
    Стандартное отклонение > 1.3 = ответы непоследовательны.
    Возвращает True если ответы консистентны.
    """
...
async def show_consistency_warning(callback: types.CallbackQuery, stage_key: str):
    """Предупреждение при хаотичных ответах — предлагает пройти этап заново"""
    vec = VECTORS[stage_key]
    stage_num = STAGE_ORDER.index(stage_key) + 1

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text="🔄 Пройти этап заново",
            callback_data=f"retry_stage_{stage_key}"
        )],
        [InlineKeyboardButton(
            text="Продолжить как есть →",
            callback_data=f"force_stage_result_{stage_key}"
        )]
    ])
...
async def show_stage_feedback(callback: types.CallbackQuery, stage_key: str):
    ...
    # ── существующий блок уточняющих вопросов ───────────────────────────────
...
async def start_command(message: types.Message):
    ...
    # ✅ НОВЫЙ стартовый экран с именем пользователя и обновлённым текстом
...
async def callback_handler(callback: types.CallbackQuery):
    ...
    # ── НОВЫЕ ВЕТКИ ДЛЯ ПРОВЕРКИ КОНСИСТЕНТНОСТИ ────────────────────────────
    elif data.startswith("retry_stage_"):
        # Пользователь хочет пройти этап заново
        stage_key = data.replace("retry_stage_", "")
        user_data[user_id]["scores"][stage_key] = []
        user_data[user_id]["current_stage"] = stage_key
        user_data[user_id]["current_question"] = 0
        # Сбрасываем флаги уточнения
        ...
        # Пользователь продолжает несмотря на предупреждение
        stage_key = data.replace("force_stage_result_", "")
        # Принудительно вызываем show_stage_feedback минуя check_consistency
...
async def show_ai_analysis(callback: types.CallbackQuery):
    """Краткий AI-анализ (улучшенный промпт)"""
    user_id = callback.from_user.id
    scores = {k: round(mean(v), 1) for k, v in user_data[user_id]["scores"].items()}
    ...
    bottleneck_pattern = LIFE_PATTERNS.get(bottleneck_key, {}).get(bottleneck_lvl, "")[:200]

    prompt = f"""Профиль человека:
{profile_summary}

Узкое место: {VECTORS[bottleneck_key]['name']} — уровень {bottleneck_lvl}/6
Паттерн узкого места: {bottleneck_pattern}

Взаимосвязи между векторами: {correlation_text}

Задача: напиши анализ в 3-4 предложения.
1. Ключевая особенность этого профиля
2. Главная проблема которая из него следует
3. Первый конкретный шаг

Обращайся на "вы". Без воды. Только суть."""
...
async def show_ai_recommendations(callback: types.CallbackQuery):
    """Показывает AI-рекомендации (улучшенный промпт)"""
    user_id = callback.from_user.id
    scores = {k: round(mean(v), 1) for k, v in user_data[user_id]["scores"].items()}
    ...
    recs_text = "\n".join([f"- {r}" for r in existing_recs]) if existing_recs else "нет"

    prompt = f"""Профиль человека:
{profile_summary}

Узкое место: {VECTORS[bottleneck_key]['name']} — уровень {bottleneck_lvl}/6
Базовые рекомендации для этого уровня:
{recs_text}

Задача: дай 3 конкретных действия которые человек может начать делать прямо сейчас.
Формат: каждое действие — 1-2 предложения, конкретно, без воды.
Обращайся на "вы". Учитывай весь профиль — не только узкое место."""
