...
ПОЛНАЯ ВЕРСИЯ с ВЫБОРОМ РЕЖИМА, КОНФАЙНМЕНТ-МОДЕЛЬЮ и МАРШРУТАМИ
...
# ══════════════════════════════════════════════
#  КАТЕГОРИИ ПОМОЩИ (ТВОЙ ВТОРОЙ МОЗГ)
# ══════════════════════════════════════════════
...
# ══════════════════════════════════════════════
#  КОНСТАНТЫ ЭТАПОВ (ваши существующие)
# ══════════════════════════════════════════════
...
async def text_to_speech(text: str, mode: str = "medium") -> bytes:
    ...
    clean_text = re.sub(r'\[(.*?)\]\(.*?\)', r'\1', clean_text)  # убираем ссылки
...
class RoutePlanner:
    """Планировщик маршрутов на основе профиля и целей"""
    
    @staticmethod
    async def create_route(user_id: int, category_id: str, subcategory_id: str) -> dict:
        """Создает персональный маршрут для пользователя"""
...
ВАЖНО: Учитывай профиль и ограничения пользователя. Если какие-то векторы слабые, давай подготовительные шаги."""
    ...
    def _get_fallback_plan(subcategory_id: str, mode: str) -> str:
        """Запасной план если API недоступен"""
        plans = {
...
async def show_introduction(message: types.Message):
    """Показывает приветствие с предложением теста"""
...
# ══════════════════════════════════════════════
#  ФУНКЦИИ ПОРТРЕТА (БЕЗ АББРЕВИАТУР)
# ══════════════════════════════════════════════
...
async def show_help_subcategory(callback: types.CallbackQuery, subcategory_id: str):
    """Показывает информацию о подкатегории и создает маршрут"""
    user_id = callback.from_user.id
...
async def route_step_done(callback: types.CallbackQuery):
    """Отмечает выполнение шага маршрута"""
    user_id = callback.from_user.id
...
# ══════════════════════════════════════════════
#  ФУНКЦИИ ЭТАПОВ ТЕСТА (ваши существующие)
# ══════════════════════════════════════════════
...
Обращайся к человеку по имени ({user_name}), если это уместно."""
        ...
        # Отправляем голосовой ответ (без форматирования)
...
async def start_command(message: types.Message):
    ...
    await show_introduction(message)
...
async def show_results(callback: types.CallbackQuery):
    ...
    # Формируем понятный портрет без аббревиатур
...
async def handle_message(message: types.Message):
    ...
    if user.get("stage") == "awaiting_question":
        ...
        # Если пользователь уже прошел тест, отправляем его к портрету
        if is_test_completed(user):
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🧠 К ПОРТРЕТУ", callback_data="show_results")],
                [InlineKeyboardButton(text="🎯 ЧЕМ ПОМОЧЬ", callback_data="show_help")],
